...
9. 통합·교열이 완료되면 챕터 배관·백업 파일(`*_draft*.md`, `*_review*.md`, `*_revised*.md`, `*_final_v*.md`)을 `{slug}/chapters/_archive/`로 무손실 이동한다. 먼저 미완료 이동 저널이 없는지 확인하고, 있으면 이전 이동을 완전히 롤백한다. 이어 정확한 대상·목적지, 일반 파일 여부, 심볼릭 링크, 기존 목적지 충돌을 **사전 검사**한다. 각 이동 전에 `원본 → 목적지`와 예상 바이트를 저널에 저장하고, 하나라도 실패하면 저널을 역순으로 **롤백**한다. 전 항목의 이름·개수·바이트가 일치할 때만 완료 처리하며 현재 `{NN}_final.md`만 `chapters/` 직하에 유지한다. 원본을 삭제하거나 기존 아카이브 파일을 덮어쓰지 않는다.
10. 팀을 해체한다.
...
GA3 보고 전 `{slug}/chapters/_archive/`와 활성 `{NN}_final.md` 목록을 확인한다. 통합 원고가 참조하는 final이 누락되었거나, 배관·백업 파일이 활성 경로에 남았거나, 아카이브가 사전 검사·롤백 가능성·무손실 검증을 통과하지 못하면 빌드를 시작하지 않는다.
