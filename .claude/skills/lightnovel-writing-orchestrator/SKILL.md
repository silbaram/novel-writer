...
**절차:**
1. `story-bible-reviewer`를 호출한다. 이 Phase에서는 **챕터 플랜과 스토리 바이블의 충돌 점검**이 목적임을 프롬프트에 명시한다. 8개 기존 검토 축 중 챕터 플랜 관련 항목(갈등 정합성, Canon 상태, 시즌 씨앗 연결)을 집중 점검하고 결과를 반환한다
2. 오케스트레이터가 결과를 읽은 뒤 `continuity-keeper`를 호출한다. 챕터 플랜 수준의 복선 흐름과 캐릭터 상태 연속성을 사전 확인하도록 프롬프트에 명시한다
3. 두 에이전트의 결과를 `{slug}/05_review_log.md`에 통합 기록한다
4. Critical 문제가 있으면 사용자에게 보고하고 해당 Phase(3 또는 4)를 재실행한다. Should 이하는 리뷰 로그에 기록하고 진행한다
