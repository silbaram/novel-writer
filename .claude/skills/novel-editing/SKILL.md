...
> **호환성 원칙:** 이 스킬의 최종 산출물(`{slug}/P05_manuscript/04_manuscript.md` + `{slug}/P05_manuscript/book_manifest.json`)은 `epub-builder` → `epub-build/scripts/build_epub.sh` 파이프라인으로 처리된다. 포맷 호환성을 항상 유지한다.
...
| 파일 | 확인 내용 |
|------|----------|
| `{slug}/P04_continuity/sNN/chapters/{CCC}_final.md` (전체) | 모든 챕터 최종본이 존재하는지 확인 |
| `{slug}/P04_continuity/continuity_log.md` | 미해결 Critical 경고가 있는지 확인 |
| `{slug}/P04_continuity/character_state_table.md` | 최신 인물 상태 확인 |
| `{slug}/P04_continuity/foreshadowing_tracker.md` | 미회수 복선은 내부 트래커용으로만 유지하고, 독자 노출 대상이 아님을 확인 |
| `{slug}/P02_bible/02_story_bible.md` | 작품 제목 후보, 저자, 언어 확인 |
| `{slug}/P03_planning/sNN/season_bible.md` (전체 시즌) | 시즌 제목, 시즌 역할 확인 |
...
수정이 필요한 경우 `{slug}/P00_meta/logs/editor_notes.md`에 전환부 수정 제안을 기록하고 결과에 명시한다. 오케스트레이터가 이를 읽어 문장·전환 리듬 문제는 `chapter-prose-reviser`, 사건·연속성 구조 문제는 `chapter-novelist`로 라우팅한다. 에디터가 직접 본문을 재서술하지 않는다.
...
- 변경 내용은 `{slug}/P00_meta/logs/editor_notes.md`에 기록

**내용 변경 금지:** 용어 표기 통일 외에 문장 재서술이나 플롯 수정을 하지 않는다. 구조·내용 수정 제안은 `P00_meta/logs/editor_notes.md`에 메모만 한다.

---

## 절차 4 — 시즌 원고 생성

각 시즌마다 `{slug}/P05_manuscript/sNN/season_manuscript.md`를 생성한다.
...
## 절차 5 — 전체 통합 원고 생성 (`P05_manuscript/04_manuscript.md`)

`epub-builder`가 직접 소비하는 파일이다. `P05_manuscript/04_manuscript.md`의 최상위 구조를 따른다.
...
## 절차 6 — `P05_manuscript/book_manifest.json` 생성
...
| `cover_image` | `"P06_publication/assets/cover.png"` | 고정 경로 |
...
### 완전한 `P05_manuscript/book_manifest.json` 예시
...
- Critical 미해결 항목이 있으면 `season_manuscript.md`와 `P05_manuscript/04_manuscript.md` **초안 생성은 허용**한다
- 같은 경우 `P00_meta/logs/build_log.md`에 경고를 명시하고 오케스트레이터에 **수동 확인 또는 수정 완료 확인**을 요청한다
...
| 요청 | 처리 |
|------|------|
| 일부 챕터 갱신 | 해당 챕터 섹션만 교체, 시즌 원고 동기화, `P05_manuscript/04_manuscript.md` 전체 재저장 |
| 서문·작가 후기 수정 | 해당 섹션만 수정 |
| 버전 업 | `P05_manuscript/book_manifest.json`의 `version` 증가 (패치: `1.0.1`, 마이너: `1.1.0`) |
