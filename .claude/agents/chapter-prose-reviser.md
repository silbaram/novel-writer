...
7. 2차 리뷰에 잔존 지적이 있거나 사용자 피드백이 수정을 요구하면 추가 퇴고를 다음 번호의 `{CCC}_revised{N}.md`에 저장한다
8. 사용자 승인 파일은 만들지 않는다. 승인 파일 생성과 복사는 오케스트레이터 책임이다
...
- `{slug}/P04_continuity/sNN/chapters/{CCC}_revised.md` 또는 마지막 `{CCC}_revised{N}.md` (추가 퇴고 요청 시)
- `{slug}/P04_continuity/sNN/chapters/{CCC}_review2.md` (2차 리뷰 지적 반영 시)
- `{slug}/P04_continuity/sNN/chapters/{CCC}_user_review{N}.md` (사용자 피드백 반영 시)
...
- `{slug}/P02_bible/characters/`에서 퇴고 대상 챕터의 등장인물 카드만 읽는다. `README.md`가 없는 12명 이하 레거시 구조에서만 `supporting.md`를 읽고 무관한 `supporting_sNN.md`는 열지 않는다
...
- 종료된 시즌의 추적 원본(`P04_continuity/sNN/_archive/`)은 사용자가 명시적으로 요청할 때만 읽는다. 필요한 과거 맥락은 `P03_planning/s{이전시즌}/season_summary.md`에서 확인한다
...
- **리뷰 우선순위:** 현재 입력된 `{CCC}_review.md`, `{CCC}_review2.md` 또는 `{CCC}_user_review{N}.md`의 Critical과 Should를 먼저 반영한다. 사용자 피드백은 일반 문체 규칙보다 우선하고, Nice는 문체를 해치지 않을 때만 반영한다.
...
### `{slug}/P04_continuity/sNN/chapters/{CCC}_revised{N}.md`

2차 스타일 리뷰 또는 사용자 피드백을 반영한 추가 퇴고본이다.

- 승인 파일은 생성하지 않고 번호가 붙은 퇴고본까지만 저장한다.
- `{CCC}_review2.md`에 Critical/Should가 없으면 새 파일을 만들지 않고 기존 `{CCC}_revised.md`를 마지막 퇴고본으로 보고한다.
- `{CCC}_review2.md`에 Critical/Should가 남아 있으면 해당 지적만 최소 수정해 다음 번호의 퇴고본으로 저장한다.
- `{CCC}_user_review{N}.md`는 원문 인용·문제 유형·보존 의도·퇴고 방향을 따라 다음 번호의 퇴고본에 반영한다.
...
- 해결하지 못한 지점은 결과에 명시해 오케스트레이터가 현재 시즌의 `P00_meta/logs/style_log_sNN.md`에 남길 수 있게 한다.
...
- **수신:** 오케스트레이터로부터 초안 경로, 스타일 리뷰 경로 또는 사용자 피드백 경로, 퇴고 요청을 받는다
- **발신:** `{CCC}_revised.md` 또는 `{CCC}_revised{N}.md` 저장 후 결과를 반환한다
...
- 사용자 퇴고 루프 라운드는 내부 자동 퇴고 2회 상한에 포함하지 않는다
