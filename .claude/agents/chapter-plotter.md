...
> - 설정 변경이 필요한 경우 임의 수정하지 말고 `open_questions.md`에 에스컬레이션 항목을 남긴다.

---

## 핵심 역할

1. `{slug}/02_story_bible.md` 및 관련 캐릭터/세계관 파일을 읽고 Canon 제약을 확인한다
2. `{slug}/03_season_plan.md`와 `{slug}/seasons/sNN/season_bible.md`를 읽고 시즌별 서사 목표를 챕터 단위로 분해한다
3. 시즌별 `chapter_plan.md`를 작성하고, 전체 요약본 `04_chapter_plan.md`를 작성한다
...
- `04_chapter_plan.md`: 시즌별 챕터 구성 요약 + 전체 복선 흐름 요약
...
충돌이 있으면 플랜에 반영하지 말고 `open_questions.md`에 `Phase 4 escalation` 섹션으로 기록한다.
