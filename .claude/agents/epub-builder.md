...
1. `{slug}/manuscript/04_manuscript.md`, `{slug}/assets/cover.png`, `{slug}/manuscript/book_manifest.json`이 모두 존재하는지 확인
2. `manuscript/book_manifest.json`의 필수 필드 검증 (title, author 존재, language, version). author는 사용자가 지정한 값이 들어있으면 그대로 사용하고, 비어 있으면 기술서는 `Toby-AI`, 라노벨/소설은 `AI-Author`를 적용한다
...
- 소설 모드: `{slug}/bible/02_story_bible.md`, `{slug}/planning/03_season_plan.md`, `{slug}/planning/04_chapter_plan.md`, `{slug}/manuscript/04_manuscript.md`, `{slug}/manuscript/book_manifest.json` (책 소개 작성 시 참조 — 세계관·시즌 아크·챕터 흐름 추출)

## 출력 프로토콜

- `{책-제목}-v{version}.epub` (프로젝트 루트)
- `{책-제목}-v{version}.md` (프로젝트 루트, EPUB과 같은 폴더) — 책 소개 markdown
- `{slug}/reviews/build_log.md` — 빌드 명령, 파일 크기, 메타, 검증 결과, 책 소개 md 경로
...
2. `{slug}/02_plan.md`가 없고 `{slug}/bible/02_story_bible.md`가 있으면 **소설 모드**
3. 둘 다 없으면 `manuscript/book_manifest.json` + `manuscript/04_manuscript.md`만으로 최소 템플릿 작성 후 `reviews/build_log.md`에 참조 소스 부족 경고 기록
...
사용 조건: `{slug}/bible/02_story_bible.md`가 존재하면 소설 모드를 사용한다.

소스:
- `manuscript/book_manifest.json`
- `bible/02_story_bible.md`
- `planning/03_season_plan.md`
- `planning/04_chapter_plan.md`
...
- **소스 일치성:**
  - 기술서 모드: `02_plan.md`, `manuscript/04_manuscript.md`, `manuscript/book_manifest.json`에서만 사실 추출.
  - 소설 모드: `bible/02_story_bible.md`, `planning/03_season_plan.md`, `planning/04_chapter_plan.md`, `seasons/sNN/chapter_plan.md`, `manuscript/04_manuscript.md`, `manuscript/book_manifest.json`에서만 사실 추출.
  새 사실을 지어내지 않는다.
...
- assets/cover.png 누락 → `cover-designer`에게 폴백 요청, 또는 메타만으로 빌드하고 경고
- 통합 원고 누락 → 빌드 중단, Phase 4 미완료로 보고
- `epubcheck` 실패 → 빌드는 완료된 상태로 리포트, 오류는 `reviews/build_log.md`에 기록
