...
1. `{slug}/P05_manuscript/04_manuscript.md`, `{slug}/P06_publication/assets/cover.png`, `{slug}/P05_manuscript/book_manifest.json`이 모두 존재하는지 확인
2. `P05_manuscript/book_manifest.json`의 필수 필드 검증 (title, author 존재, language, version). author는 사용자가 지정한 값이 들어있으면 그대로 사용하고, 비어 있으면 기술서는 `Toby-AI`, 라노벨/소설은 `AI-Author`를 적용한다
...
- 소설 모드: `{slug}/P02_bible/02_story_bible.md`, `{slug}/P03_planning/03_season_plan.md`, `{slug}/P03_planning/04_chapter_plan.md`, `{slug}/P05_manuscript/04_manuscript.md`, `{slug}/P05_manuscript/book_manifest.json` (책 소개 작성 시 참조 — 세계관·시즌 아크·챕터 흐름 추출)

## 출력 프로토콜

- `{책-제목}-v{version}.epub` (프로젝트 루트)
- `{책-제목}-v{version}.md` (프로젝트 루트, EPUB과 같은 폴더) — 책 소개 markdown
- `{slug}/P00_meta/logs/build_log.md` — 빌드 명령, 파일 크기, 메타, 검증 결과, 책 소개 md 경로
...
2. `{slug}/02_plan.md`가 없고 `{slug}/P02_bible/02_story_bible.md`가 있으면 **소설 모드**
3. 둘 다 없으면 `P05_manuscript/book_manifest.json` + `P05_manuscript/04_manuscript.md`만으로 최소 템플릿 작성 후 `P00_meta/logs/build_log.md`에 참조 소스 부족 경고 기록
...
사용 조건: `{slug}/P02_bible/02_story_bible.md`가 존재하면 소설 모드를 사용한다.

소스:
- `P05_manuscript/book_manifest.json`
- `P02_bible/02_story_bible.md`
- `P03_planning/03_season_plan.md`
- `P03_planning/04_chapter_plan.md`
- `P05_manuscript/04_manuscript.md`
...
- **저자:** {author}
- **장르:** {genre}
- **시즌/권:** {season_or_volume}
- **언어:** {language}
- **버전:** v{version}

## 작품 소개

{2~4문단. 작품의 핵심 갈등, 주제, 관전 포인트를 스포일러 없이 소개한다.}

## 장르/톤

{장르, 톤, 무드, 페이싱을 3~6줄로 요약한다.}

## 세계관

{2~4문단 또는 bullet. 02_story_bible.md에서 세계 규칙·세력·시대 배경을 요약한다.}
...
{수록 시즌/권 범위 요약 + 포함 챕터 수. 03_season_plan.md/04_chapter_plan.md 기준.}
...
- **본문 문체 강요 안 함:** 기술서 본문은 기술서용 문체를 따르고, 소설 본문은 `style-guides/lightnovel-style-guide.md`와 스토리 바이블의 캐릭터 목소리를 따른다. 책 소개는 두 경우 모두 일반 마케팅 톤(존중·간결·정보 중심)이 더 적합하다. 다만 과장·홍보성 클리셰("당신의 인생이 바뀝니다")는 피한다.
- **소스 일치성:**
  - 기술서 모드: `02_plan.md`, `P05_manuscript/04_manuscript.md`, `P05_manuscript/book_manifest.json`에서만 사실 추출.
  - 소설 모드: `P02_bible/02_story_bible.md`, `P03_planning/03_season_plan.md`, `P03_planning/04_chapter_plan.md`, `P03_planning/sNN/chapter_plan.md`, `P05_manuscript/04_manuscript.md`, `P05_manuscript/book_manifest.json`에서만 사실 추출.
  새 사실을 지어내지 않는다.
...
- P06_publication/assets/cover.png 누락 → `cover-designer`에게 폴백 요청, 또는 메타만으로 빌드하고 경고
- 통합 원고 누락 → 빌드 중단, Phase 4 미완료로 보고
- `epubcheck` 실패 → 빌드는 완료된 상태로 리포트, 오류는 `P00_meta/logs/build_log.md`에 기록
