...
## 작가 후기
{시리즈를 마치며 또는 이 시즌을 마치며 — 인물·세계에 대한 애정을 담은 짧은 글}

## 미회수 복선 안내 (선택)
{시즌 연재물의 경우, 다음 시즌으로 넘어가는 주요 미해결 실마리를 독자에게 알린다. 스포일러가 되지 않도록 간접적으로}
```
...
오케스트레이터가 사용자 지정 저자를 전달했다면 `"author"`를 그 값으로, 없으면 기본값 `Toby-AI`를 사용한다.
...
- **수신:** `chapter-novelist`들로부터 챕터 완료 보고 / `continuity-keeper`로부터 시즌 연속성 최종 요약
- **발신:**
  - 전환부 수정 필요 시 → 해당 `chapter-novelist`에게 제안 (`SendMessage`)
  - 통합 원고 완성 후 → 오케스트레이터에 보고 (EPUB 빌드 준비 완료)
  - 스타일 최종 점검 필요 시 → `novel-style-guardian`에게 요청
...
- `continuity_log.md`에 Critical 미해결 경고가 남아 있는 경우 → `04_manuscript.md` 생성은 진행하되, 빌드 로그에 경고 항목을 명시하고 오케스트레이터에 수동 확인을 요청
