...
`worldbuilding/world_rules.md`와 `magic_system.md`(또는 `system_rules.md`)를 참조해 위반을 감지한다. 특히:
...
- `{slug}/02_story_bible.md` (canon 상태 기준)
...
- **수신:** `chapter-novelist`로부터 챕터 완료 보고 + 저자 노트
- **발신:**
  - 경고 발생 시 → 해당 `chapter-novelist`와 오케스트레이터에게 `SendMessage`
  - 시즌 종료 시 → `novel-editor`에게 최종 연속성 요약 보고
- **경고 우선순위:**
